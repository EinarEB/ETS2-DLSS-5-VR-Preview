...
### Installed for testing on 2026-09-07 at 23:19

Both branch add-ons from commit a28f69d were installed into `E:\ETS2-VR-Preview` with `tools\install_branch_build.py`; every hash recorded in `preview.json` was re-verified afterwards (64 files, no mismatch). The previous add-ons and `preview.json` are in `E:\ETS2-VR-Preview\branch-backups\20260907-231918\`. `dlss5-feed.cfg` was not changed: it still has `stereo_eye_shift=-608`, one pass, band split on; the new keys take their defaults (gains 1, no intensity write) until the feeder saves the file, so the first start looks exactly like the milestone 2 build. The new controls are in the Add-ons tab under DLSS 5 Feed: "Detail gain (%)" and "Tone gain (%)" below "Filter tone and detail separately", and "Cab strength (%)" below them. The headset steps are listed above.
