...
//
// Requires <windows.h>, feed_driver_api.h and version.lib. Header-only and static: both
...
{
    const auto& api = feed_driver::Get();
    log("[%s] NGX direct driver: %ls; ready=%d; Windows error=%lu", tag, api.path.c_str(), api.ready, api.error);
    log("[%s] NGX feature-requirements preflight is not queried in this variant; actual init/create/evaluate results follow. Model DLLs must be beside the game EXE.", tag);
}
