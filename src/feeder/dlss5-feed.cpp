...
//
// ETS2 preview derivative: accepts only the windowless D3D11 OpenXR runtime.
// The upstream multi-API transport remains below for source comparison; its
// other entry paths are disabled in this preview.
//
...
// This ETS2 preview loads the registered NVIDIA driver through the MIT bridge
// interface. No NVIDIA SDK library or implementation is linked into the add-on.
//
...
#include "feed_sbs_blit.h" // private equal-eye SBS bilinear expansion; mono entry stays unchanged
#include "feed_residual.h" // opt-in SDR native-detail composition of the combined DLAA + NR difference
#include "feed_stability.h"
#include "feed_d3d11_context.h"
#include "feed_context_protection.h"
#include "feed_profile11.h"
// Normal device/session callbacks release these resources explicitly. Retain the
// managers themselves so CRT teardown cannot call COM after process termination
// has stopped other threads or torn down another driver DLL.
static auto& g_profile11=*new ets2_profile::Profiler;
static auto& g_private11=*new ets2_d3d11::PrivateState;
static auto& g_context_protection=*new ets2_d3d11::ProtectionRegistry;
static auto& g_stability=*new ets2_stability::Filter;
static uint64_t g_stability_epoch=0;
static bool g_delivered_since_present=false;
static bool g_stability_applied=false;

#ifdef ETS2_FEED_SEQUENCE_REPLAY
#define FEED_VERSION "0.13.1-beta.1-ets2-stereo.14-sequence-fixture"
#define ETS2_CAPTURE_SOURCE "offline_recorded_consecutive_sequence"
#else
#define FEED_VERSION "0.13.1-beta.1-ets2-stereo.21-driver-preview1"
#define ETS2_CAPTURE_SOURCE "real_matched_depth_and_estimated_motion"
#endif

extern "C" __declspec(dllexport) const char *NAME = "DLSS 5 Feed " FEED_VERSION;
extern "C" __declspec(dllexport) const char *DESCRIPTION =
    "ETS2 VR test: supplies both eye images, matched depth and estimated motion to neural rendering. "
    "Pass count, resolution, final blend and flicker filter are here. "
    "Style and intensity are in the separate DLSS 5 Neural Rendering panel.";
...
#include "feed_stereo_diagnostics.h"
static void FeedCaptureGpuDiagnostic(const char* operation, HRESULT result);
#include "feed_capture.h"
#include "feed_temporal_capture.h"
#include "feed_native_observer.h"
#ifdef ETS2_FEED_SEQUENCE_REPLAY
#include "feed_sequence_replay.h"
#endif
#ifdef ETS2_FEED_PACKET_REPLAY
#include "feed_packet_replay.h"
#endif
...
// One owned driver session per process. The direct shutdown export is not used:
// prior isolated probes faulted there even following successful SDK initialization.
// Keep the device/session alive as NIGos does, and require restart after closure.
static volatile LONG g_preview_restart_required = 0;
static ID3D12Device* g_preview_retained_device = nullptr;
static NVSDK_NGX_Parameter* g_preview_retained_parameters = nullptr;
static bool PreviewNeedsRestart(){return InterlockedCompareExchange(&g_preview_restart_required,0,0)!=0;}
...
struct Cfg
{
    ...
    int   stereo_diagnostics; // local metadata logging only, 0 off / 1 on; parse-only
    int   stereo_mode; // experimental: 1 = equal SBS eyes, separate features; D3D11, ordinary DLAA work scaling
    int   stereo_depth_required; // 1 = feed only when companion reports a verified depth pair
    int   work_composite; // 0 = existing replacement; 1 = preserve native detail with combined pipeline residual (SDR D3D11)
    float work_mix; // 0..1 strength of that difference; 0 preserves the untouched native image exactly
    int test_compare_hotkey; // opt-in Ctrl+Alt+N comparison for the foreground game and windowless runtime
    int stereo_carrier_copy; // restart-only: preserve game AA input, skip extra SR computation
    int stereo_passes; // restart-only: one or two genuine NR passes per eye
    int stereo_controls=1; // coordinate live controls across all owned contexts
    float stereo_second_tone=1.0f; // multiply user's tone in pass two only
    float stereo_second_structure=1.0f; // multiply user's structure in pass two only
...
static int       g_stereo_launch_resolution = 0; // first successful stereo build; restart-only
static int       g_pending_work_resolution = 0;
static ULONGLONG g_work_resolution_apply_after = 0;
static std::string StabilitySnapshot(){
    return std::string("\"stability\":{\"applied\":")+(g_stability_applied?"true":"false")+
        ",\"history_used\":"+(g_stability_applied&&g_stability.UsedHistory()?"true":"false")+
        ",\"strength\":"+std::to_string(g_cfg.stereo_stability)+"}";
}
...
static void CfgWriteDefault()
{
    ...
    fprintf(f,
            ...
            g_cfg.work_resolution, g_cfg.work_upscale, g_cfg.work_sharpness, g_cfg.work_composite, g_cfg.work_mix, g_cfg.test_compare_hotkey,
...
static bool CfgReload()
{
    ...
    while (fgets(line, sizeof(line), f) != nullptr)
    {
        ...
        if (!std::isfinite(val) || val < -2147483648.0f || val >= 2147483648.0f) continue;
        ...
        else if (_stricmp(key, "work_composite") == 0 && (val == 0.0f || val == 1.0f)) next.work_composite = iv;
        else if (_stricmp(key, "work_mix") == 0) next.work_mix = val;
        else if (_stricmp(key, "test_compare_hotkey") == 0 && (val == 0.0f || val == 1.0f)) next.test_compare_hotkey = iv;
        ...
        else if (_stricmp(key, "stereo_diagnostics") == 0) next.stereo_diagnostics = iv;
        else if (_stricmp(key, "stereo_mode") == 0) next.stereo_mode = iv;
        else if (_stricmp(key, "stereo_depth_required") == 0) next.stereo_depth_required = iv;
        else if (_stricmp(key, "stereo_carrier_copy") == 0 && (val==0 || val==1)) next.stereo_carrier_copy=iv;
        else if (_stricmp(key, "stereo_passes") == 0 && (val==1 || val==2)) next.stereo_passes=iv;
        else if (_stricmp(key, "stereo_controls") == 0 && (val==0 || val==1)) next.stereo_controls=iv;
        else if (_stricmp(key, "stereo_second_tone") == 0 && val>=0 && val<=1) next.stereo_second_tone=val;
        else if (_stricmp(key, "stereo_second_structure") == 0 && val>=0 && val<=1) next.stereo_second_structure=val;
        else if (_stricmp(key, "stereo_float_color") == 0 && (val==0 || val==1)) next.stereo_float_color=iv;
    ...
    if (next.work_mix < 0.0f || next.work_mix > 1.0f) next.work_mix = g_cfg.work_mix;
    ...
    if (next.stereo_diagnostics != 0 && next.stereo_diagnostics != 1) next.stereo_diagnostics = g_cfg.stereo_diagnostics;
    if (next.stereo_mode != 0 && next.stereo_mode != 1) next.stereo_mode = g_cfg.stereo_mode;
    if (next.stereo_depth_required != 0 && next.stereo_depth_required != 1) next.stereo_depth_required = g_cfg.stereo_depth_required;
    ...
    const bool rebuild = next.hdr != g_cfg.hdr || next.depth_inverted != g_cfg.depth_inverted ||
                         ...
                         next.mode != g_cfg.mode || next.stereo_mode != g_cfg.stereo_mode || next.stereo_depth_required != g_cfg.stereo_depth_required;
    ...
    Log("[feed] config: enabled=%d mode=%d hdr=%d depth_inverted=%d flags=%d reset_every=%d warmup_rebuild=%d "
        "rebuild=%d log_frames=%d create_delay=%d work_resolution=%d%% work_upscale=%d work_sharpness=%.2f work_composite=%d work_mix=%.3f test_compare_hotkey=%d gpu_timeout_ms=%d buffer_home=%d async_home=%d sync_home=%d mv_scale=%.3f,%.3f stall_log_ms=%d",
        g_cfg.enabled, g_cfg.mode, g_cfg.hdr, g_cfg.depth_inverted, g_cfg.flags, g_cfg.reset_every,
...
static const struct { int mode; const char *file, *tech; } kMvProviders[] = {
    ...
    { 5, "lumenite_Kernel_SBS.fx", "Lumenite_Kernel_SBS" },
    { 6, "ETS2_VortStereo.fx", "ETS2_VortStereo" },
};
static const char *kMvModeName[] = { "texMotionVectors", "Launchpad", "VORT", "LumeniteFX Kernel", "LumeniteFX QuantMotion", "LumeniteFX Kernel SBS (private experiment)", "ETS2 Vort stereo" };
...
struct Feed
{
    ...
    uint64_t                   ts_profile_id[kFrames]{};
    bool                       ts_pending[kFrames]{};
    ...
    ID3D11ShaderResourceView *residual_input_srv; // exact stored input, before DLAA and NR
    bool residual_failed, residual_announced;
    bool stereo_sdr_float;
    const char *residual_reason; // bounded diagnostic, reset at resource generation change
    ...
    ID3D11PixelShader  *blit_sbs_ps;
    ID3D11PixelShader  *residual_ps;
    ID3D11Buffer       *residual_cb;
    ...
    float  fsr_sharpness;
    int    fsr_stereo;
    bool   unfenced_submission; // stereo: submitted work whose retirement cannot be proved
...
static Feed g;
#include "feed_preview_runtime.h"
static bool StereoResourcesOwned(); // defined with the private eye state below
...
{
    if(g.dev12)InterlockedExchange(&g_preview_restart_required,1);
...
static void FeedCaptureGpuDiagnostic(const char* operation, HRESULT result)
{
    const HRESULT removed11 = g.dev11 ? g.dev11->GetDeviceRemovedReason() : S_OK;
    const HRESULT removed12 = g.dev12 ? g.dev12->GetDeviceRemovedReason() : S_OK;
    Log("[capture] gpu operation=%s result=0x%08X removed11=0x%08X removed12=0x%08X fence_queued=%llu fence_completed=%llu thread=%lu",
        operation, result, removed11, removed12, g.fence_value,
        g.fence12 ? g.fence12->GetCompletedValue() : 0, GetCurrentThreadId());
    if (FAILED(result) || FAILED(removed11) || FAILED(removed12)) {
        FeedDrainInfoQueue("capture failure");
        if (FAILED(removed12)) FeedDumpDred(removed12);
    }
}
...
{
    if (g.ts_read == nullptr || !g.ts_pending[slot]) return;
    ...
    if (t[base + 1] > t[base])
    {
        const double elapsed=1000.0 * double(t[base + 1] - t[base]) / double(g.ts_freq);
        g.ts_sum_ms += elapsed;
        g_profile11.NeuralTime(g.ts_profile_id[slot],elapsed);
        ++g.ts_n;
    }
    const D3D12_RANGE wrote = { 0, 0 };   // read-only map
    g.ts_read->Unmap(0, &wrote);
    g.ts_pending[slot]=false;g.ts_profile_id[slot]=0;
}
...
static UINT64 EndCommands()
{
    ...
    const HRESULT signaled = g.queue->Signal(g.fence12, v);
    CK("queue Signal(fence12)");
    g.ts_pending[g.frame_slot]=SUCCEEDED(signaled)&&g.ts_heap!=nullptr;
    g.alloc_fence[g.frame_slot] = v;
    g.frame_slot = (g.frame_slot + 1) % Feed::kFrames;
    if (FAILED(signaled) && (g_cfg.stereo_mode || StereoResourcesOwned())) {
        // ExecuteCommandLists already happened. No fence means the models and
        // every session dependency must survive even if CPU cleanup is requested.
        g.unfenced_submission = true;
        Log("[stereo] queue Signal failed 0x%08X after submission; retaining the session", signaled);
        FeedDisable("the stereo GPU submission could not be fenced; restart before resuming");
        return 0;
    }
    return v;
}
...
static bool DrainGpu()
{
    ...
    if (g.dev12 && FAILED(g.dev12->GetDeviceRemovedReason())) return true;
    const UINT64 v = ++g.fence_value;
    const HRESULT signaled = g.queue->Signal(g.fence12, v);
    if (FAILED(signaled)) {
        Log("[feed] queue drain Signal failed 0x%08X", signaled);
        return g.dev12 && FAILED(g.dev12->GetDeviceRemovedReason());
    }
    if (g.fence12->GetCompletedValue() < v)
    {
        if (!g.fence_event || FAILED(g.fence12->SetEventOnCompletion(v, g.fence_event)) ||
            WaitForSingleObject(g.fence_event, 5000) != WAIT_OBJECT_0 || g.fence12->GetCompletedValue() < v) {
            Log("[feed] could not prove queue retirement before teardown");
            return g.dev12 && FAILED(g.dev12->GetDeviceRemovedReason());
        }
    }
...
#include "feed_stereo.h"

static void ReleaseFrameResources()
{
    if (StereoRetirementPending()) {
        g.frame_ready = false;
        Log("[stereo] keeping session resources: a submitted create has not retired");
        return;
    }
    const bool drained = DrainGpu();
    if (!drained && (g_cfg.stereo_mode || StereoResourcesOwned())) {
        g.unfenced_submission = true;
        g.frame_ready = false;
        FeedDisable("stereo GPU work has not retired; session resources are retained until process cleanup");
        return;
    }
    StereoRelease();
    ...
    SafeRelease(g.residual_input_srv);
    g.residual_failed = g.residual_announced = false;
    g.residual_reason = nullptr;
...
{
    if (g.blit_vs != nullptr && g.blit_ps != nullptr && g.blit_sbs_ps != nullptr && g.resample_ps != nullptr &&
    ...
    ID3DBlob *sbs = nullptr;
    hr = compile(kSbsBlitSrc, sizeof(kSbsBlitSrc) - 1, "feedsbsblit", nullptr, nullptr, "ps_sbs", "ps_5_0", 0, 0, &sbs, &err);
    if (FAILED(hr)) {
        Log("[feed] SBS blit PS compile failed 0x%08X: %s", hr, err ? (const char *)err->GetBufferPointer() : "");
        SafeRelease(err); SafeRelease(vs); SafeRelease(ps); SafeRelease(resample); return false;
    }
    SafeRelease(err);
    ...
    if (SUCCEEDED(hr)) hr = g.dev11->CreatePixelShader(sbs->GetBufferPointer(), sbs->GetBufferSize(), nullptr, &g.blit_sbs_ps);
...
{
    InterlockedExchange(&g_preview_restart_required,1);
    Log("[preview] repeated feature creation failure; close and restart ETS2. The driver session is retained, not shut down or initialized twice.");
    return false;
}
...
static bool OnCreateFeatureFailed(bool crashed)
{
    ...
    {
        FeedDisable("close and restart ETS2 after repeated neural feature creation failures");
        return false;
    }
...
static bool RecreateFeatureOnly(UINT w, UINT h)
{
    ...
    // Reusing FP16 storage must also retain its established SDR interpretation.
    // Inferring HDR from that storage format breaks same-size VR recreation.
    g.hdr = (g_stereo.active && g.stereo_sdr_float) ? false : (g_cfg.hdr >= 0 ? g_cfg.hdr != 0 : IsHdrFormat(g.color_fmt));
    if (g_stereo.active) return StereoRecreate(inverted);
...
{
    const bool floatColor=g_cfg.stereo_mode&&g_cfg.stereo_float_color;
    const auto nativeFormat=TypedColorFormat(bb_fmt);
    if(floatColor&&((nativeFormat!=DXGI_FORMAT_R8G8B8A8_UNORM&&nativeFormat!=DXGI_FORMAT_B8G8R8A8_UNORM)||
       g_cfg.hdr==1||(g_cfg.flags>=0&&(g_cfg.flags&NVSDK_NGX_DLSS_Feature_Flags_IsHDR))||
       !g_cfg.work_composite||!g_cfg.stereo_carrier_copy)){
        FeedDisable("floating-point stereo color requires preserved SDR game input and native-detail composition");return false;
    }
    if(g_stereo.active&&floatColor!=g.stereo_sdr_float){
        FeedDisable("the saved color-precision choice requires a full game restart");return false;
    }
    if (g_cfg.stereo_mode && (w < 2 || (w & 1) || backbuffer_w < 2 || (backbuffer_w & 1) || g_cfg.work_upscale == 2)) {
        FeedDisable("stereo processing requires equal even-width eyes and ordinary bilinear or FSR work scaling");
        return false;
    }
    ...
    if (g.session_ready && g_cfg.mode >= 2 && g.feature != nullptr && g.tex12[SLOT_COLOR] != nullptr &&
        g_stereo.active == (g_cfg.stereo_mode != 0) &&
    ...
    if (StereoRetirementPending()) return false;
    ...
    g.bb_fmt     = bb_fmt;
    g.stereo_sdr_float=floatColor;
    g.color_fmt  = floatColor?DXGI_FORMAT_R16G16B16A16_FLOAT:nativeFormat;
    g.output_fmt = ResolveOutputFormat(g.color_fmt, g.dev12);
    // Storage precision never changes the source's transfer function. The FP16
    // path contains the same display-encoded SDR values as the original RGBA8.
    g.hdr        = floatColor?false:(g_cfg.hdr >= 0 ? g_cfg.hdr != 0 : IsHdrFormat(g.color_fmt));
    const bool inverted = g_cfg.depth_inverted >= 0 ? g_cfg.depth_inverted != 0 : g.depth_reversed;
    ...
    if (backbuffer_w != w || backbuffer_h != h || g.stereo_sdr_float)
    {
        ...
        ss.Format              = nativeFormat;   // copy remains in the original backbuffer format
...
static bool CreateDlssFeature(UINT w, UINT h, bool inverted, bool *crashed)
{
    if (g_cfg.stereo_mode) return StereoBuild(w, h, inverted, crashed);
...
{
    if(PreviewNeedsRestart()){FeedDisable("close and restart ETS2 to open a fresh neural session");return false;}
    if (StereoRetirementPending()) return false;
    if(!g_context_protection.Acquire(ctx))return false;
    ...
    FeedAttachInfoQueue();
        ...
        Log("[feed] D3D11 context protection remains attached for the game device lifetime");
...
{
    // Latch BEFORE retirement checks. An unretired old session must never be
    // reused after the game changes devices, even while its resources survive.
    InterlockedExchange(&g_preview_restart_required,1);
    if (StereoRetirementPending()) {
        Log("[stereo] shutdown retains unretired GPU resources; process cleanup owns them if the fence never completes");
        return;
    }
    ReleaseFrameResources();
    if (StereoRetirementPending()) return; // a failed drain discovered during release retains dependencies too
    // Transfer this one owned reference to process-lifetime retention. The
    // feature/texture path above still drains and releases its own resources.
    // Raw retained pointers intentionally have no COM/driver destructor.
    if(g.params && !g_preview_retained_parameters)g_preview_retained_parameters=g.params;
    g.params=nullptr;
    if(g.dev12 && !g_preview_retained_device)g_preview_retained_device=g.dev12;
    g.dev12=nullptr;g.ngx_inited=false;
    SafeRelease(g.blit_vs);
    SafeRelease(g.blit_ps);
    SafeRelease(g.blit_sbs_ps);
    SafeRelease(g.residual_ps);
    SafeRelease(g.residual_cb);
    g_stability.Release();
    g_private11.Release();
    g_profile11.Release();
    for(unsigned i=0;i<Feed::kFrames;++i){g.ts_pending[i]=false;g.ts_profile_id[i]=0;}
...
{
    FeedDisable("the ETS2 preview does not initialize a game-owned D3D12 session");return false;
    if (StereoRetirementPending()) return false;
...
{
    FeedDisable("the ETS2 preview does not initialize Vulkan sessions");return false;
    if (StereoRetirementPending()) return false;
...
{
    FeedDisable("the ETS2 preview does not initialize OpenGL sessions");return false;
    if (StereoRetirementPending()) return false;
                                 ...
                                 ID3D11ShaderResourceView *mask_srv, UINT source_w, UINT source_h)
{
    ...
    if(!isolation){Log("[feed] input state isolation failed 0x%08X",isolation.Status());return false;}
    ...
    if (source_w != g.width || source_h != g.height || g.stereo_sdr_float)
    {
...
static void UpdateFsrConstants(ID3D11DeviceContext *ctx, UINT in_w, UINT in_h)
{
    if (in_w == g.fsr_in_w && in_h == g.fsr_in_h && g.backbuffer_width == g.fsr_out_w &&
        g.backbuffer_height == g.fsr_out_h && g_cfg.work_sharpness == g.fsr_sharpness && g_cfg.stereo_mode == g.fsr_stereo) return;
    D3D11_MAPPED_SUBRESOURCE mapped = {};
    ...
    static_cast<FsrConstants *>(mapped.pData)->rcas[3] = g_cfg.stereo_mode ? 1.0f : 0.0f;
    ...
    g.fsr_stereo = g_cfg.stereo_mode;
}
...
#include "feed_residual_composite.h"

static bool BlitOutputToBackbuffer(ID3D11DeviceContext *ctx, ID3D11RenderTargetView *rtv)
{
    // A live config edit cannot bypass native periphery preservation while a
    // cropped feature from this launch is still active.
    if (g_cfg.work_composite || (g_stereo.active && g_stereo.crop_percent > 0))
        return CompositeOutputToBackbuffer(ctx, rtv);
    ets2_d3d11::Scope isolation(g_private11,ctx);
    if(!isolation){Log("[feed] output state isolation failed 0x%08X",isolation.Status());return false;}
    ...
    if (rcas || !easu)
    {
        ...
        ctx->OMSetRenderTargets(1, target, nullptr);
        ctx->PSSetShader(rcas ? g.rcas_ps : (g_cfg.stereo_mode ? g.blit_sbs_ps : g.blit_ps), nullptr, 0);
        ctx->PSSetShaderResources(0, 1, &src);
        ctx->Draw(3, 0);
    }
...
#include "feed_residual_backend.h"
...
static void FeedFrame12(reshade::api::effect_runtime *rt, reshade::api::command_list *cl, reshade::api::resource_view rtv)
{
    ...
    if (!ResidualBackendAllowsReplacement()) return;
...
static void FeedFrame11(reshade::api::effect_runtime *rt, reshade::api::command_list *cl, reshade::api::resource_view rtv)
{
    ...
    // A reload can change the contract after FeedFrame's entry check. Reject
    // it here too, before bindings, resource changes or driver calls this frame.
    if (g_cfg.stereo_mode != 1 || g_cfg.stereo_depth_required != 1 ||
        g_cfg.work_composite != 1 || g_cfg.work_upscale != 0) {
        FeedDisable("restore the VR preview configuration and restart ETS2");
        return;
    }
    LatchPreviewBlend(rt);
    if (!g_cfg.enabled || g_cfg.mode == 0) return;
    ...
    const auto singleTexture=[](const D3D11_TEXTURE2D_DESC& d){return d.Width&&d.Height&&d.MipLevels==1&&d.ArraySize==1&&d.SampleDesc.Count==1&&d.SampleDesc.Quality==0;};
    g.mask_ok = mask != nullptr && singleTexture(kd) && kd.Width == cd.Width && kd.Height == cd.Height && kd.Format == DXGI_FORMAT_R8_UNORM;

    bool ok = true;
    if (cd.Width != md.Width || cd.Height != md.Height || cd.Width != dd.Width || cd.Height != dd.Height ||
        !singleTexture(cd) || !singleTexture(md) || !singleTexture(dd) || md.Format != DXGI_FORMAT_R16G16_FLOAT || dd.Format != DXGI_FORMAT_R32_FLOAT)
    {
        static bool said = false;
        if (!said)
        {
            ...
            Log("[feed] whole-texture copies require one mip, layer and sample: color %u/%u/%u, motion %u/%u/%u, depth %u/%u/%u",cd.MipLevels,cd.ArraySize,cd.SampleDesc.Count,md.MipLevels,md.ArraySize,md.SampleDesc.Count,dd.MipLevels,dd.ArraySize,dd.SampleDesc.Count);
        }
        ok = false;
    }
    ...
    if (ok)
    {
        ...
        else
        {
            ...
            if (PreviewNeedsRestart()) {FeedDisable("close and restart ETS2 after the render device or neural session changed");ok=false;}
            else if (!g.session_ready) ok = InitSession(dev, ctx);
            dev->Release();
        }
    }
    ...
    const int work_percent=(g_cfg.stereo_mode&&g_stereo_launch_resolution)?g_stereo_launch_resolution:g_cfg.work_resolution;
    const bool sr_wanted = g_cfg.work_upscale == 2 && g_cfg.mode >= 2 && work_percent < 100;
    const UINT work_w = sr_wanted ? ScaledExtentUp(cd.Width,  work_percent) : ScaledExtent(cd.Width,  work_percent);
    const UINT work_h = sr_wanted ? ScaledExtentUp(cd.Height, work_percent) : ScaledExtent(cd.Height, work_percent);
    ...
    const bool needs_build11 = !g.frame_ready || work_w != g.width || work_h != g.height ||
                               ...
                               FeatureMissingForMode();
    if (g_stereo.active && (work_w != g.width || work_h != g.height || cd.Format != g.bb_fmt)) {
        // The exact classic consumer crashes in the controlled active-NR resize
        // test. A fixed-size prototype must not trigger that known-bad lifecycle.
        FeedDisable("the stereo image size/format changed; restart the game before resuming neural rendering");
        g.need_reset = true;
        ok = false;
    }
    // Re-arm the grace on a resolution/format change too: that makes the DLSS 5 add-on
    ...
    if (ok && needs_build11)
    {
        Log("[feed] building: %ux%u work resolution (%d%%) -> %ux%u backbuffer %s (mv %s, depth %s, depth reversed=%d)",
            work_w, work_h, work_percent, cd.Width, cd.Height, FormatName(cd.Format),
...
#endif
    bool captureStarted=false,temporalStarted=false;
    const auto captureFrame=g.frames_done+1;
    const auto captureDeliveredBefore=g.frames_done;
    if(ok && g_cfg.stereo_mode && (ets2_capture::Get().WantsFrame()||ets2_temporal::Get().WantsFrame())){
        try{
            ...
            meta<<"{\"build\":\"" FEED_VERSION "\",\"source\":\"" ETS2_CAPTURE_SOURCE "\",\"capture_stage\":\"Feeder input and output, before any later ReShade effects\",\"work_width\":"<<g.width<<",\"work_height\":"<<g.height<<",\"work_percent\":"<<work_percent<<",\"saved_work_percent\":"<<g_cfg.work_resolution
                ...
                <<",\"jitter_x\":"<<(g.sr_active?g.jitter_x:0.0f)<<",\"jitter_y\":"<<(g.sr_active?g.jitter_y:0.0f)
                <<",\"host_process\":"<<ets2_capture::Quote(ets2_capture::ProcessName())<<",\"guide_uniforms\":"<<ets2_temporal::GuideSettings(rt)<<",\"saved_configuration_is_not_live_parameter_proof\":true"
#ifdef ETS2_FEED_SEQUENCE_REPLAY
                <<",\"sequence_replay\":"<<ets2_sequence_replay::Metadata()
#endif
                <<"}";
            D3D11_RENDER_TARGET_VIEW_DESC captureView{};rtv11->GetDesc(&captureView);
            ets2_d3d11::Scope captureScope(g_private11,ctx);
            if(!captureScope)throw std::runtime_error("capture context isolation unavailable");
            if(ets2_temporal::Get().WantsFrame())temporalStarted=ets2_temporal::Get().Begin(ctx,captureFrame,color,unsigned(captureView.Format),depth,mv,mask,meta.str(),rt);
            else captureStarted=ets2_capture::Get().Begin(ctx,captureFrame,color,unsigned(captureView.Format),depth,mv,mask,meta.str());
        }catch(...){ets2_capture::Get().Cancel("Capture setup failed; game rendering continues.");ets2_temporal::Get().Cancel("Motion capture setup failed; game rendering continues.");}
    }
    if (ok)
    {
        ...
        if(g_stereo.active&&g_cfg.mode>=2&&!captureStarted&&!temporalStarted&&!ets2_capture::Get().Busy()&&!ets2_temporal::Get().Busy())g_profile11.Begin(ctx,g_stability_epoch,g.frames_done+1,g_log_path);
        ...
        ok = CopyOrResampleInputs(ctx, color, mv, depth, mask,
                                  ...
                                  cd.Width, cd.Height);
        if(ok)g_profile11.Mark(ctx,ets2_profile::InputDone);

        if (ok && g_cfg.mode == 1)
        {
            // Transport test: what went out comes straight back, through the same copy-back path.
            {
                ets2_d3d11::Scope restore(g_private11,ctx);
                ok=bool(restore);
                if(ok)ctx->CopyResource(g.tex11[SLOT_OUTPUT], g.tex11[SLOT_COLOR]);
            }
            if(ok)ok=BlitOutputToBackbuffer(ctx, rtv11);
            if(ok)++g.frames_done;else g.need_reset=true;
        }
        else if (ok)
        {
            const UINT64 v_in = ++g.fence_value;
            const HRESULT input_signal = g.ctx4->Signal(g.fence11, v_in);
            if (FAILED(input_signal) && g_cfg.stereo_mode) {
                Log("[stereo] input Signal failed 0x%08X; no D3D12 wait was queued", input_signal);
                FeedDisable("the stereo input fence failed; current image retained");
                g.need_reset = true; ok = false;
            } else {
                ctx->Flush();
                const HRESULT input_wait = g.queue->Wait(g.fence12, v_in);
                if (FAILED(input_wait) && g_cfg.stereo_mode) {
                    Log("[stereo] input queue Wait failed 0x%08X; neural work skipped", input_wait);
                    FeedDisable("the stereo input queue wait failed; current image retained");
                    g.need_reset = true; ok = false;
                }
            }

            if (ok && !BeginCommands()) { FeedFail("command list"); ok = false; }
            else if (ok)
            {
                g.ts_profile_id[g.frame_slot]=g_profile11.ActiveId();
                Barrier(g.tex12[SLOT_COLOR],  D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
                Barrier(g.tex12[SLOT_DEPTH],  D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
                Barrier(g.tex12[SLOT_MV],     D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
                if (g.mask_ok) Barrier(g.tex12[SLOT_MASK], D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
                ...
                DWORD ecode = 0;
                NVSDK_NGX_Result re = g_stereo.active ? StereoEvaluate(&ep, &ecode) : SafeEvaluateDLSS(&ep, &ecode);
                ...
                if (v_out == 0) {
                    Log("[feed] evaluate completion unavailable; keeping the original image (submitted without a fence=%d)", g.unfenced_submission ? 1 : 0);
                    g.need_reset = true; g.frame_ready = false; ok = false;
                }
                else if (NVSDK_NGX_FAILED(re))
                {
                    Log("[feed] evaluate failed 0x%08X (%s)", re, NgxResultName(re));
                ...
                else
                {
                    ...
                    const HRESULT output_wait = g.ctx4->Wait(g.fence11, v_out);
                    if (FAILED(output_wait) && g_cfg.stereo_mode) {
                        Log("[stereo] output Wait failed 0x%08X; processed image not copied", output_wait);
                        FeedDisable("the stereo output wait failed; current image retained");
                        g.need_reset = true; ok = false;
                    } else if(g_stereo.active&&!ets2_native::CanDeliverControls()){
                        g.need_reset=true;ok=false;
                        static unsigned controlSkips=0;
                        if(controlSkips++<3||controlSkips%300==0)Log("[stereo-controls] incomplete/inconsistent owned controls; original frame retained, all contexts retry with common reset");
                    } else {
                    g_profile11.Mark(ctx,ets2_profile::ReturnReady);
                    ok=BlitOutputToBackbuffer(ctx, rtv11);
                    g_profile11.Mark(ctx,ets2_profile::OutputDone);
                    if(!ok){g.need_reset=true;g_stability.Invalidate();}
                    else {
                    const UINT64 n = ++g.frames_done;
                    if(IsPreviewRuntime(rt))g_preview_blend.Delivered(g_preview_frame,n,GetTickCount64());
                    g.consecutive_fails = 0;
                    if (g.sr_active) ++g.jitter_index;
                    if (n <= static_cast<UINT64>(g_cfg.log_frames) || (n % 1800) == 0)
                        Log("[feed] frame %llu delivered (%ux%u at %d%% -> %ux%u, reset=%d%s, jitter %+.3f,%+.3f)", n,
                            g.width, g.height, work_percent,
    ...
    if(g_profile11.ActiveId()){
        const auto& policy=ets2_native::Controls();const auto& control=policy.Current();
        std::ostringstream config;
        config<<"{\"native_width\":"<<g.backbuffer_width<<",\"native_height\":"<<g.backbuffer_height<<",\"work_width\":"<<g.width<<",\"work_height\":"<<g.height<<",\"passes\":"<<g_stereo.eye_count/2<<",\"preserved_carrier\":"<<(g_stereo.copy_carrier?"true":"false")<<",\"fp16\":"<<(g.stereo_sdr_float?"true":"false")<<",\"stability\":"<<g_cfg.stereo_stability<<",\"blend\":"<<g_frame_work_mix<<",\"composite\":"<<g_cfg.work_composite<<",\"second_tone\":"<<g_cfg.stereo_second_tone<<",\"control_values\":[";
        for(unsigned i=0;i<control.values.size();++i){if(i)config<<',';config<<control.values[i];}config<<"],\"second_structure\":"<<g_cfg.stereo_second_structure<<",\"control_modes\":[";
        for(unsigned i=0;i<control.modes.size();++i){if(i)config<<',';config<<control.modes[i];}config<<"],\"creation_serials\":[";
        for(unsigned i=0;i<policy.Creation().size();++i){if(i)config<<',';config<<policy.Creation()[i];}config<<"]}";
        g_profile11.Finish(ctx,ok&&g.frames_done>captureDeliveredBefore,policy.CommonReset(),config.str());
    }
    if(g_stereo.active)ets2_native::FinishControls(ok&&g.frames_done>captureDeliveredBefore);
    if(captureStarted){
        try{ets2_d3d11::Scope captureScope(g_private11,ctx);if(!captureScope)throw std::runtime_error("capture context isolation unavailable");ets2_capture::Get().End(ctx,ok&&g.frames_done>captureDeliveredBefore,color,g.tex11[SLOT_COLOR],g.tex11[SLOT_DEPTH],g.tex11[SLOT_MV],g.tex11[SLOT_OUTPUT],g.mask_ok?g.tex11[SLOT_MASK]:nullptr,"\"observer\":"+ets2_native::Snapshot()+","+StabilitySnapshot());}
        catch(...){ets2_capture::Get().Cancel("Capture output failed; game rendering continues.");}
    }
    if(temporalStarted){
        try{ets2_d3d11::Scope captureScope(g_private11,ctx);if(!captureScope)throw std::runtime_error("capture context isolation unavailable");ets2_temporal::Get().End(ctx,ok&&g.frames_done>captureDeliveredBefore,color,"\"observer\":"+ets2_native::Snapshot()+","+StabilitySnapshot());}
        catch(...){ets2_temporal::Get().Cancel("Motion capture output failed; game rendering continues.");}
    }
#ifdef ETS2_FEED_SEQUENCE_REPLAY
    if(ok&&g.frames_done>captureDeliveredBefore)ets2_sequence_replay::Delivered();
#endif
    SafeRelease(color);
    SafeRelease(mv);
    SafeRelease(depth);
...
{
    if (StereoRetirementPending()) {
        FeedDisable("a submitted stereo create has not retired; restart after the GPU has recovered");
        return;
    }
    if ((g_cfg.stereo_mode || StereoResourcesOwned()) && rt->get_device()->get_api() != reshade::api::device_api::d3d11) {
        FeedDisable("experimental stereo mode is implemented only for the D3D11 transport");
        return;
    }
    if(rt->get_device()->get_api()==reshade::api::device_api::d3d11)FeedFrame11(rt,cl,rtv);
    else FeedDisable("this preview supports the ETS2 D3D11 OpenXR renderer only");
}
...
#include "feed_test_compare_runtime.h"

// The private paired-depth test belongs to ReShade's windowless VR runtime.
// A desktop preset reload or headset sleep must never move it to the mirror.
static bool AcceptTestRuntime(reshade::api::effect_runtime *rt)
{
    return rt && !rt->get_hwnd() && rt->get_device()->get_api()==reshade::api::device_api::d3d11;
}
static LONG g_test_depth_status = 0; // 0: waiting, 1: missing, 2: paired

#include "stereo_motion_runtime.h"
static void FeedFrame(reshade::api::effect_runtime *rt, reshade::api::command_list *cl, reshade::api::resource_view rtv)
{
    if (!AcceptTestRuntime(rt)) return;
    if (!FeedEnter()) return;   // logs the dropped call, with its thread id
    ++g_stability_epoch;g_stability_applied=false;
    if(PreviewNeedsRestart()){FeedDisable("close and restart ETS2 to resume neural rendering");FeedLeave();return;}
    if(g_cfg.stereo_mode!=1||g_cfg.stereo_depth_required!=1||g_cfg.work_composite!=1||g_cfg.work_upscale!=0){
        FeedDisable("restore the VR preview configuration; this release requires paired stereo depth and native detail composition");FeedLeave();return;
    }
    if(!g_cfg.enabled||g.disabled||g_cfg.mode==0){g_stability.Invalidate();FeedLeave();return;}
    if(rt->get_device()->get_api()==reshade::api::device_api::d3d11){
        auto* context=reinterpret_cast<ID3D11DeviceContext*>(cl->get_native());
        // The depth companion runs earlier in the effect chain. Its first
        // unprotected attempt fails closed; the next frame can prove depth.
        if(!g_context_protection.Acquire(context)){g_stability.Invalidate();g.need_reset=true;FeedLeave();return;}
    }
    char observeOnly[8]={};
    if(GetEnvironmentVariableA("ETS2_FEED_OBSERVE_ONLY",observeOnly,sizeof(observeOnly))&&strcmp(observeOnly,"1")==0){
        static bool announced=false;if(!announced){Log("[lab] observe-only: depth/effects active; no Feeder image processing or NGX submission");announced=true;}
        FeedLeave();return;
    }
    FeedThreadTrace();
    if(g_cfg.stereo_mode && rt->get_device()->get_api()==reshade::api::device_api::d3d11){
        try{
            ets2_native::Poll(g_self);
            auto* captureContext=reinterpret_cast<ID3D11DeviceContext*>(cl->get_native());
            if(captureContext && captureContext->GetType()==D3D11_DEVICE_CONTEXT_IMMEDIATE){
                if(!ets2_temporal::Get().Busy())ets2_capture::Get().Poll(captureContext,g.frames_done);
                ets2_temporal::Get().Poll(captureContext,ets2_capture::Get().Busy());
            }
        }catch(...){Log("[capture] diagnostic polling failed; image processing continues");}
    }
    if (TestComparisonSkipsFeed(rt)) { FeedLeave(); return; }
    if (g_cfg.stereo_diagnostics) LogStereoDepthContract(rt, rtv);
    if (g_cfg.stereo_depth_required) {
        // The matching add-on refreshes this before effects on EVERY VR frame.
        // Missing shader support, stale/missing/ambiguous candidates or lost depth
        // leave the current image untouched and force a common history reset.
        bool ready = false;
        const auto variable = rt->find_uniform_variable(kEffectFile, "ETS2_STEREO_DEPTH_VALID");
        if (variable.handle) rt->get_uniform_value_bool(variable, &ready, 1);
        InterlockedExchange(&g_test_depth_status, ready ? 2 : 1);
        static bool last_ready = false;
        static unsigned skipped = 0;
        if (!ready) {
            // This gate runs before backend reloads and frames_done is frozen.
            // Keep file changes recoverable while retaining this unverified frame.
            static ULONGLONG next_depth_poll = 0;
            PollBlockedFeedConfig(next_depth_poll, GetTickCount64());
            g.need_reset = true;
            if (skipped++ % 300 == 0 || last_ready)
                Log("[stereo] verified depth pair unavailable; neural feed skipped, current image retained (uniform %s)", variable.handle ? "present" : "missing");
            if(rt->get_device()->get_api()==reshade::api::device_api::d3d11 && ets2_capture::Get().WantsFrame()){
                try{
                    auto* context=reinterpret_cast<ID3D11DeviceContext*>(cl->get_native());
                    auto resource=rt->get_device()->get_resource_from_view(rtv);
                    Microsoft::WRL::ComPtr<ID3D11Texture2D> original;
                    if(context&&resource.handle&&SUCCEEDED(reinterpret_cast<ID3D11Resource*>(resource.handle)->QueryInterface(IID_PPV_ARGS(&original)))){
                        D3D11_RENDER_TARGET_VIEW_DESC view{};reinterpret_cast<ID3D11RenderTargetView*>(rtv.handle)->GetDesc(&view);
                        const std::string meta="{\"neural_evaluated\":false,\"depth_pair_verified\":false,\"reason\":\"Stereo depth adapter rejected this frame before NGX initialization.\",\"image_stage\":\"Current image at Feed; earlier preset effects may be present.\",\"host_process\":"+ets2_capture::Quote(ets2_capture::ProcessName())+",\"observer\":"+ets2_native::Snapshot()+"}";
                        ets2_capture::Get().Blocked(context,GetTickCount64(),original.Get(),unsigned(view.Format),meta);
                    }
                }catch(...){ets2_capture::Get().Cancel("Original-image diagnostic failed; game rendering continues.");}
            }
            last_ready = false;
            FeedLeave();
            return;
        }
        if (!last_ready) Log("[stereo] verified depth pair available; resuming with both histories reset");
        last_ready = true; skipped = 0;
    }
    if (ReadMvProviderMode(rt)==6) {
        bool motionReady=false;
        const auto variable=rt->find_uniform_variable(kEffectFile,"ETS2_MOTION_HISTORY_VALID");
        if(variable.handle)rt->get_uniform_value_bool(variable,&motionReady,1);
        if(!motionReady) {
            static ULONGLONG nextMotionPoll=0;
            PollBlockedFeedConfig(nextMotionPoll,GetTickCount64());
            g.need_reset=true;g_stability.Invalidate();
            static unsigned skippedMotion=0;
            if(skippedMotion++%300==0)Log("[stereo] waiting for consecutive current-frame stereo motion; source image retained");
            FeedLeave();return;
        }
    }
    const auto framesBefore=g.frames_done;
    FeedFrameDispatch(rt, cl, rtv);
    if(g.frames_done!=framesBefore)g_delivered_since_present=true;
    FeedLeave();
}

// ReShade emits this after every runtime presentation, including frames where
// the user disabled a technique or all effects. The next image's optical flow
// cannot reproject history across those missing deliveries, even if the pause
// was shorter than the time-based history guard.
#include "feed_vr_look.h"
static void OnRuntimePresent(reshade::api::effect_runtime* rt)
{
    MotionRuntimePresented(rt);
    if(rt!=g.runtime||!AcceptTestRuntime(rt)||!FeedEnter())return;
    const bool delivered=g_delivered_since_present;
    PollPreviewKey(rt);
    PublishPreviewStatus(rt,delivered);
    const LONG lookChoice=delivered?InterlockedExchange(&g_vr_look_pending,-1):-1;
    const bool lookChanged=lookChoice>=0;
    if(!g_delivered_since_present){g_stability.Invalidate();g.need_reset=true;}
    if(lookChanged){g_stability.Invalidate();g.need_reset=true;}
    g_delivered_since_present=false;
    FeedLeave();
    if(delivered)ApplyPendingVrLook(rt,lookChoice);
    SavePendingVrImage(rt,delivered,lookChanged);
}
...
static void ResolveHandles(reshade::api::effect_runtime *rt)
{
    ...
    // The private paired-depth branch can explicitly use a different convention
    // from Generic Depth. Its presence is determined from the compiled texture,
    // so a disabled branch never changes the ordinary provider's NGX contract.
    if (rt->find_texture_variable(kEffectFile, "ETS2_StereoDepthInput").handle &&
        (rt->get_preprocessor_definition_for_effect(kEffectFile, "DLSS5_STEREO_DEPTH_REVERSED", v) ||
         rt->get_preprocessor_definition("DLSS5_STEREO_DEPTH_REVERSED", v)))
        g.depth_reversed = atoi(v) != 0;
...
static void KeepVrCallbackCodeMapped(reshade::api::effect_runtime* rt)
{
    static HMODULE pinned=nullptr;
    if(pinned || !rt || rt->get_hwnd()!=nullptr || rt->get_device()->get_api()!=reshade::api::device_api::d3d11)return;
    auto** table=*reinterpret_cast<void***>(rt);
    HMODULE owner=nullptr;
    if(!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS|GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                         reinterpret_cast<LPCWSTR>(table[0]),&owner))return;
    if(!GetProcAddress(owner,"ReShadeRegisterAddon") || !GetProcAddress(owner,"ReShadeUnregisterAddon"))return;
    if(GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS|GET_MODULE_HANDLE_EX_FLAG_PIN,
                        reinterpret_cast<LPCWSTR>(table[0]),&pinned)){
        wchar_t path[MAX_PATH]={};GetModuleFileNameW(pinned,path,MAX_PATH);
        Log("[lifetime] VR callback code retained until process exit: %p %ls",pinned,path);
    }
}

static void OnInitEffectRuntime(reshade::api::effect_runtime *rt)
{
    if(AcceptTestRuntime(rt)){
        static HMODULE pinned=nullptr;
        if(!pinned&&!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS|GET_MODULE_HANDLE_EX_FLAG_PIN,
             reinterpret_cast<LPCWSTR>(&OnInitEffectRuntime),&pinned)){
            FeedDisable("could not retain the VR callback module safely");return;
        }
    }
    if(g_cfg.stereo_mode)KeepVrCallbackCodeMapped(rt);
    ets2_capture::Get().Initialize(g_self);
    ets2_temporal::Get().Initialize(g_self);
    ...
    if (!AcceptTestRuntime(rt))
    {
        if (inits <= 8) Log("[test] desktop runtime tracked but excluded from the VR depth test");
        return;
    }
    ...
    if (rebind)
    {
        ...
        if(g.runtime!=rt)g_preview_blend.ResetRuntime();
        g.runtime = rt;
        ResolveHandles(rt);
...
{
    ResetMotionRuntime(rt);
    if(rt->get_hwnd()==nullptr)ets2_capture::Get().Cancel("VR runtime ended before capture completed.");
    if(rt->get_hwnd()==nullptr)ets2_temporal::Get().Cancel("VR runtime ended before motion capture completed.");
    ...
    if (!was_bound) return;
    g_preview_blend.ResetRuntime();
    g_delivered_since_present=false;g_stability.Invalidate();g.need_reset=true;
    InterlockedExchange(&g_test_depth_status, 0);
...
{
    if (!g_cfg.enabled || !AcceptTestRuntime(rt)) return;
    MotionProviderCompleted(rt,technique);
    if (rt != g.runtime)
    {
...
static void OnDestroyDevice(reshade::api::device *dev)
{
    ...
    if(dev->get_api()==reshade::api::device_api::d3d11)
        g_context_protection.ReleaseDevice(reinterpret_cast<ID3D11Device*>(dev->get_native()));
}
...
static bool RequestVrTrace()
{
    ...
    if (!end) return false;
    const size_t space = MAX_PATH - (end + 1 - path);
    if (wcscpy_s(end + 1, space, L"lineage-record.request") != 0) return false;
    char nonce[64] = {};
    const int length = sprintf_s(nonce, "overlay_%lu_%llu", GetCurrentProcessId(), GetTickCount64());
    ...
    DWORD written = 0;
    const bool ok = WriteFile(file, nonce, length, &written, nullptr) && written == static_cast<DWORD>(length);
    CloseHandle(file);
    if (ok) Log("[test] four-frame trace requested from Add-ons: %s", nonce);
    return ok;
}

static void DrawOverlay(reshade::api::effect_runtime *rt)
{
    // The monitor's overlay can run on a different thread from VR. Config edits
    // and their blend snapshots share the frame lock; never wait on the renderer
    // from an overlay callback, since it may hold another runtime's own lock.
    struct OverlayScope {
        bool entered = false;
        OverlayScope() {
            if (!TryEnterCriticalSection(&g_feed_cs)) return;
            if (g_feed_busy) { LeaveCriticalSection(&g_feed_cs); return; }
            g_feed_busy = true; entered = true;
        }
        ~OverlayScope() { if (entered) FeedLeave(); }
    } scope;
    if (!scope.entered) {
        ImGui::TextUnformatted("VR frame is updating. Controls will return on the next frame.");
        return;
    }
    if (g_cfg.stereo_mode == 1 && g_cfg.stereo_depth_required)
    {
        ...
        if(ImGui::CollapsingHeader("Observed VR inputs and neural settings")){
            ets2_native::Draw(g.frames_done,g_stereo.active?g_stereo.eye_count/2:0);
            if(g.runtime){char presetPath[2048]{};g.runtime->get_current_preset_path(presetPath);ImGui::TextWrapped("Active neural preset: %s",presetPath[0]?presetPath:"not available");}
        }
        const LONG depth = InterlockedCompareExchange(&g_test_depth_status, 0, 0);
        if (depth == 1)
            ImGui::TextColored(ImVec4(1.0f, 0.65f, 0.2f, 1.0f), "VR depth pair missing: neural rendering has no verified input.");
        else if (depth == 2)
            ImGui::TextUnformatted("VR depth pair found. Processing status is below.");
        else
            ImGui::TextUnformatted("Waiting for the VR scene to render.");
        ImGui::Separator();
    }
    bool dirty = false;
    ...
    bool rebuild = false;
    if (g_cfg.stereo_mode == 1 && g_cfg.stereo_depth_required && g_cfg.work_composite)
    {
        g_preview_blend.Configure(g_cfg.work_mix);
        const auto comparison = g_preview_blend.Status();
        float blend_percent = comparison.selected.value * 100.0f;
        if (ImGui::SliderFloat("Final effect blend (%)", &blend_percent, 0.0f, 100.0f, "%.0f%%"))
        {
            g_cfg.work_mix = blend_percent * 0.01f;
            g_preview_blend.Manual(g_cfg.work_mix);
            dirty = true;
        }
        const int keys[] = {145,19,0};
        int keyChoice=2;
        for(int i=0;i<3;++i)if(keys[i]==g_cfg.preview_toggle_key)keyChoice=i;
        if(ImGui::Combo("Compare original / neural",&keyChoice,"Scroll Lock\0Pause\0Disabled\0")){
            g_cfg.preview_toggle_key=keys[keyChoice];g_preview_blend.Disarm();dirty=true;
        }
        ImGui::TextWrapped("Press the comparison key while the game has focus: 0%% / 100%%. Neural processing stays active. The comparison lasts for this session; moving the blend slider saves a new starting blend.");
        if(comparison.temporary)ImGui::TextUnformatted("Temporary comparison is active.");
        if(comparison.deliveredFrame && GetTickCount64()-comparison.deliveredTick<2000 && comparison.delivered.serial==comparison.selected.serial)
            ImGui::Text("Delivered to VR image: %.0f%%",comparison.delivered.value*100);
        else ImGui::TextUnformatted("Waiting for a VR frame with the selected blend.");
        ImGui::TextWrapped("100%% applies the full neural edit. 0%% removes it; other enabled ReShade effects still apply. This blend does not change neural Intensity, pass count or cost.");
    }
    if(g_cfg.stereo_mode==1){
        int selected=ets2_quality::Match(g_cfg.stereo_passes,g_cfg.work_resolution,g_cfg.stereo_crop)+1;
        if(ImGui::Combo("Quality preset (restart to apply)",&selected,"Custom\0Low\0Medium\0High\0Ultra\0")&&selected>0){
            const auto& quality=ets2_quality::presets[selected-1];
            g_cfg.stereo_passes=quality.passes;g_cfg.work_resolution=quality.resolution;g_cfg.stereo_crop=quality.crop;
            g_cfg.stereo_carrier_copy=1;g_cfg.stereo_float_color=1;g_cfg.work_composite=1;g_cfg.work_upscale=0;
            g_work_resolution_ui=g_cfg.work_resolution;g_pending_work_resolution=0;dirty=true;
        }
        ImGui::TextWrapped("Low: small square, 50%%, one pass. Medium: larger square, 65%%, one pass. High: broad square, 80%%, two passes. Ultra: full image, 100%%, two passes. Style, intensity and model stay as you set them.");
        bool cropped=g_cfg.stereo_crop!=0;
        if(ImGui::Checkbox("Process a centered square per eye (restart)",&cropped)){g_cfg.stereo_crop=cropped?75:0;dirty=true;}
        if(cropped&&ImGui::SliderInt("Square size (% of shorter eye side) - restart",&g_cfg.stereo_crop,40,100)){dirty=true;}
        ImGui::TextWrapped("Outside the square, the original game image is preserved. An 8%% soft edge blends the neural edit into the surrounding image. This is centered on each view, not eye tracking. 100%% square still crops a rectangular eye; disable cropping for the entire image.");
        if(g_stereo.active){
            int active=ets2_quality::Match(int(g_stereo.eye_count/2),g_stereo_launch_resolution,g_stereo.crop_percent);
            ImGui::Text("ACTIVE: %s | %ux%u neural input per eye | %u pass(es)",active>=0?ets2_quality::presets[active].name:"Custom",g_stereo.width,g_stereo.height,g_stereo.eye_count/2);
            bool pending=g_stereo.crop_percent!=g_cfg.stereo_crop||g_stereo_launch_resolution!=g_cfg.work_resolution||g_stereo.eye_count!=2u*unsigned(g_cfg.stereo_passes)||g.stereo_sdr_float!=(g_cfg.stereo_float_color!=0)||g_stereo.copy_carrier!=(g_cfg.stereo_carrier_copy!=0);
            if(pending)ImGui::TextColored(ImVec4(1.0f,.72f,.2f,1.0f),"RESTART REQUIRED - saved settings are not active yet.");
            else ImGui::TextUnformatted("Saved quality settings are active.");
            auto saved=ets2_quality::Region(ScaledExtent(g.backbuffer_width,g_cfg.work_resolution)/2,ScaledExtent(g.backbuffer_height,g_cfg.work_resolution),g_cfg.stereo_crop);
            double fraction=double(saved.width)*saved.height*g_cfg.stereo_passes/(double(g.backbuffer_width/2)*g.backbuffer_height*2);
            ImGui::Text("Saved preset neural pixels x passes: %.1f%% of Ultra",100.0*fraction);
            ImGui::TextDisabled("This measures neural input workload, not a predicted frame rate.");
        }
        ImGui::Separator();
        int passes=g_cfg.stereo_passes-1;
        if(ImGui::Combo("Neural passes (restart to apply)",&passes,"1 pass per eye\0" "2 passes per eye\0")){g_cfg.stereo_passes=passes+1;dirty=true;}
        if(ImGui::SliderInt("Work resolution (%) - restart to apply",&g_cfg.work_resolution,50,100)){g_work_resolution_ui=g_cfg.work_resolution;g_pending_work_resolution=0;dirty=true;}
        if(g_stereo.active){
            ImGui::Text("Active work resolution: %d%%",g_stereo_launch_resolution);
            if(g_stereo_launch_resolution!=g_cfg.work_resolution)ImGui::TextWrapped("New resolution saved for the next game launch.");
        }
        int input=g_cfg.stereo_carrier_copy?0:1;
        if(ImGui::Combo("Input processing (restart to apply)",&input,"Preserve game detail\0" "Legacy extra DLAA (diagnostic)\0")){g_cfg.stereo_carrier_copy=input==0?1:0;dirty=true;}
        bool floatColor=g_cfg.stereo_float_color!=0;
        if(ImGui::Checkbox("Keep floating-point color between passes (restart)",&floatColor)){g_cfg.stereo_float_color=floatColor?1:0;dirty=true;}
        if(g_stereo.active){
            ImGui::Text("Active color storage: %s",g.stereo_sdr_float?"floating point, SDR":"8-bit SDR");
            if(g.stereo_sdr_float!=floatColor)ImGui::TextWrapped("Color precision is saved for the next game launch.");
        }
        if(g_stereo.active){
            ImGui::Text("Active: %u neural pass(es) per eye; %s",g_stereo.eye_count/2,g_stereo.copy_carrier?"game detail preserved":"extra DLAA");
        ...
        if(coordinated){
            float tonePercent=g_cfg.stereo_second_tone*100;
            if(ImGui::SliderFloat("Tone in second pass (%)",&tonePercent,0,100,"%.0f%%")){g_cfg.stereo_second_tone=tonePercent*.01f;dirty=true;}
            ImGui::SameLine();HelpMarker("100% repeats your full Tone setting. Lower values reduce the additional color and brightness change in pass two. Both eyes reset together.");
            float structurePercent=g_cfg.stereo_second_structure*100;
            if(ImGui::SliderFloat("Structure in second pass (%)",&structurePercent,0,100,"%.0f%%")){g_cfg.stereo_second_structure=structurePercent*.01f;dirty=true;}
            ImGui::SameLine();HelpMarker("Multiplies your DLSS 5 Structure setting for the second pass only. The first pass keeps your full setting. Both eyes reset together when this changes.");
        }
    ...
    if(g_cfg.stereo_mode==1&&g_cfg.stereo_depth_required&&ImGui::CollapsingHeader("Record screenshots or motion")){
        DrawFinalVrImage();
        ImGui::Separator();
        ImGui::TextWrapped("The comparison and motion packets below record neural inputs/output before the optional color look.");
        ets2_capture::Get().Draw();
        ets2_temporal::Get().Draw();
        ImGui::TextWrapped("If depth is missing, the screenshot recorder saves the original image and failure details.");
        static int requested=0;
        if(GetModuleHandleW(L"lineage-observer.addon64")!=nullptr){
            if(ImGui::Button("Record draw metadata (advanced)"))requested=RequestVrTrace()?1:-1;
            if(requested>0)ImGui::TextWrapped("Draw trace requested. This records rendering metadata, not images; completion is in the trace log.");
            if(requested<0)ImGui::TextUnformatted("Could not write the trace request.");
        }
    }
    ...
    if (PreviewNeedsRestart())
        ImGui::TextWrapped("Neural rendering stopped. Close ETS2 completely and start it again from the preview launcher. This session cannot be restarted from the overlay.");
    else if (g.disabled && ImGui::Button("Re-enable"))
    {
        g.disabled = false;
        g.consecutive_fails = 0;
    ...
    ImGui::Separator();
    if(g_cfg.stereo_mode!=1||ImGui::CollapsingHeader("Advanced engine settings")){
    ...
    if (adjustable_work_resolution && g_cfg.stereo_mode!=1)
    {
        if (g_pending_work_resolution == 0 && g_work_resolution_ui != g_cfg.work_resolution)
            g_work_resolution_ui = g_cfg.work_resolution;
        if (ImGui::SliderInt(g_cfg.stereo_mode?"Work resolution (%) - restart to apply":"Work resolution (%)", &g_work_resolution_ui, 50, 100))
        {
            if(g_cfg.stereo_mode){g_cfg.work_resolution=g_work_resolution_ui;g_pending_work_resolution=0;dirty=true;}
            else{g_pending_work_resolution = g_work_resolution_ui;g_work_resolution_apply_after = GetTickCount64() + 400;}
        }
        ImGui::SameLine(); HelpMarker(g_cfg.stereo_mode?"Changes neural working resolution while preserving native game detail. Saved for the next game launch; the active consumer cannot resize safely.":"Scales both axes of the private work textures. Applied once 400 ms after dragging stops.");
        if (g_pending_work_resolution != 0)
            ...
            ImGui::TextDisabled("Active: %ux%u (%d%%) -> %ux%u", g.width, g.height,
                                (g_cfg.stereo_mode&&g_stereo_launch_resolution)?g_stereo_launch_resolution:g_cfg.work_resolution, g.backbuffer_width, g.backbuffer_height);
        if(g_cfg.stereo_mode&&g_stereo_launch_resolution&&g_stereo_launch_resolution!=g_cfg.work_resolution)
            ImGui::TextWrapped("Saved: %d%%. Restart the game to apply this resolution.",g_cfg.work_resolution);
        ...
        if(g_cfg.work_composite){
            ImGui::TextWrapped("Native-detail composition is active. FSR and its sharpening controls do not apply to this mode.");
        }else{
    ...
    ImGui::Separator();
    if(g_cfg.stereo_mode==1 && (g_stereo.active ? g_stereo.copy_carrier : g_cfg.stereo_carrier_copy!=0)){
        ImGui::TextWrapped("Extra DLAA is skipped. Choose Style and Intensity in the DLSS 5 Neural Rendering card.");
    }else{
    ImGui::TextUnformatted("Legacy DLAA input preset (unused when preserving game detail)");
    ImGui::TextWrapped("This controls preliminary DLAA. The neural look is selected with Style in the DLSS 5 Neural Rendering card.");
...
{
    // Generic upstream backends remain in the source for comparison, but the
    // ETS2 release never installs Vulkan hooks or initializes another backend.
    return false;
}

BOOL APIENTRY DllMain(HMODULE module, DWORD reason, LPVOID reserved)
{
    // Process termination runs under the loader lock after other threads may
    // have stopped. Do not drain GPU fences, release COM, unregister callbacks
    // or acquire logging locks here. Windows reclaims process-owned resources.
    // A dynamic unload (reserved == nullptr) still uses the cleanup below.
    ...
    if (reason == DLL_PROCESS_ATTACH)
    {
        ...
        reshade::register_event<reshade::addon_event::reshade_present>(OnRuntimePresent);
        reshade::register_event<reshade::addon_event::reshade_screenshot>(OnFinalVrImageSaved);
        reshade::register_event<reshade::addon_event::reshade_render_technique>(OnRenderTechnique);
#ifdef ETS2_FEED_SEQUENCE_REPLAY
        reshade::register_event<reshade::addon_event::reshade_finish_effects>(OnFinishEffectsForSequence);
#endif
    ...
    else if (reason == DLL_PROCESS_DETACH)
    {
        ...
        reshade::unregister_event<reshade::addon_event::reshade_present>(OnRuntimePresent);
        reshade::unregister_event<reshade::addon_event::reshade_screenshot>(OnFinalVrImageSaved);
        reshade::unregister_event<reshade::addon_event::reshade_render_technique>(OnRenderTechnique);
#ifdef ETS2_FEED_SEQUENCE_REPLAY
        reshade::unregister_event<reshade::addon_event::reshade_finish_effects>(OnFinishEffectsForSequence);
#endif
        ...
        ShutdownSession();
        g_context_protection.ReleaseDevice(nullptr);
