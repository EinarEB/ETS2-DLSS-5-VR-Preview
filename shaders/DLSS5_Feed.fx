/*
    ...
    technique ABOVE this one in the effect list:
      ...
      2  VORT                vort_Motion.fx: MotVectTexVort (MIT wrapper, CC BY-NC estimator)
...
// Private SBS experiment. Provider 5 is inherently SBS; other providers may opt in.
#ifndef DLSS5_SBS
    #define DLSS5_SBS ((DLSS5_MV_PROVIDER == 5) || (DLSS5_MV_PROVIDER == 6))
#endif
#if DLSS5_SBS && ((BUFFER_WIDTH % 2) != 0)
    #error "DLSS5_SBS requires equal-width eyes and an even combined width."
#endif

// Private opt-in matched-depth input. The capture add-on publishes a texture only
// after exact current-frame color correspondence identifies both eye snapshots.
// A paired color/depth copy already uses the native SBS pixel orientation. Generic
// Depth's flip, mirror, scale, offset and multiplier settings must not touch it.
#ifndef DLSS5_STEREO_DEPTH
    #define DLSS5_STEREO_DEPTH 0
#endif
#if DLSS5_STEREO_DEPTH
    #if !DLSS5_SBS
        #error "Matched stereo depth requires DLSS5_SBS."
    #endif
    #ifndef DLSS5_STEREO_DEPTH_REVERSED
        #define DLSS5_STEREO_DEPTH_REVERSED RESHADE_DEPTH_INPUT_IS_REVERSED
    #endif
    ...
    uniform bool ETS2_STEREO_DEPTH_VALID < hidden = true; > = false;
#endif
...
#else
    return 0.0; // Function is unused in the unchanged Generic Depth path.
#endif
}
...
float LinearDepth(float2 uv)
{
    ...
    #if DLSS5_STEREO_DEPTH_REVERSED
        depth = 1.0 - depth;
    #endif
    // Same raw source as DLSS5_Depth. This is the existing perspective-depth
    // approximation used by validation; it is not a recovered eye projection.
    return depth / (RESHADE_DEPTH_LINEARIZATION_FAR_PLANE - depth * (RESHADE_DEPTH_LINEARIZATION_FAR_PLANE - 1.0));
#else
...
#elif DLSS5_MV_PROVIDER == 6
    texture2D ETS2VortMotion { Width = BUFFER_WIDTH; Height = BUFFER_HEIGHT; Format = RG16F; };
    sampler sDLSS5_ProviderMV { Texture = ETS2VortMotion; AddressU = Clamp; AddressV = Clamp; MipFilter = Point; MinFilter = Point; MagFilter = Point; };
    uniform bool ETS2_MOTION_HISTORY_VALID < hidden = true; > = false;
    #define DLSS5_MV_PROVIDER_NAME "ETS2 Vort stereo (independent eye motion)"
...
#elif DLSS5_MV_PROVIDER == 5
    // Private isolated-eye Kernel SBS prototype; matches its even output grid exactly.
    namespace KernelSBS {
        texture2D tFlow { Width = 2 * ((BUFFER_WIDTH / 2) / 8); Height = BUFFER_HEIGHT / 8; Format = RG16F; };
        texture2D tConfidence { Width = 2 * ((BUFFER_WIDTH / 2) / 8); Height = BUFFER_HEIGHT / 8; Format = R16F; };
    }
    sampler sDLSS5_ProviderMV { Texture = KernelSBS::tFlow; AddressU = Clamp; AddressV = Clamp; MipFilter = Point; MinFilter = Linear; MagFilter = Linear; };
    sampler sDLSS5_ProviderMVPoint { Texture = KernelSBS::tFlow; AddressU = Clamp; AddressV = Clamp; MipFilter = Point; MinFilter = Point; MagFilter = Point; };
    sampler sDLSS5_ProviderConfidence { Texture = KernelSBS::tConfidence; AddressU = Clamp; AddressV = Clamp; };
    #define DLSS5_MV_PROVIDER_NAME "Experimental Kernel SBS (independent eye motion)"
    #define DLSS5_MV_LOWRES 1
...
bool MotionHistoryReady()
{
#if DLSS5_MV_PROVIDER == 6
    return ETS2_MOTION_HISTORY_VALID;
#else
    return true;
#endif
}
...
uniform int MV_PROVIDER_INFO <
    ...
    ui_text = "Motion vector provider: " DLSS5_MV_PROVIDER_NAME "\n"
              ...
              "  3 LumeniteFX Kernel   4 LumeniteFX QuantMotion   5 Private Kernel SBS   6 ETS2 Vort stereo\n"
...
uniform bool GEOM_ENABLE <
    ...
    ui_label = "Use geometry vectors (disabled when DLSS5_SBS is on)";
...
// All neighborhood taps stay in the source pixel's eye. A clamp is used only
// after central-history validity is checked; a cross-eye center is never clamped
// into a pretend valid correspondence.
...
{
#if DLSS5_SBS
    // Ordered comparisons also reject NaN/Inf before any history read.
    if (!all(abs(mv) < 1e6)) return true;
    float targetX = uv.x + mv.x;
    return uv.x < 0.5 ? targetX >= 0.5 : targetX < 0.5;
#else
    return false;
#endif
}
...
{
#if DLSS5_SBS
    // The existing fit has one camera model for the whole frame. It would couple
    // eyes even after local sampling was fixed; use provider validation instead.
    return false;
#else
    return GEOM_ENABLE;
#endif
...
// Keep interpolation inside the source eye. The output has an even grid width.
float2 ProviderSampleUV(float2 uv)
{
#if DLSS5_MV_PROVIDER == 5 || DLSS5_SBS
    float2 halfTexel = 0.5 / float2(tex2Dsize(sDLSS5_ProviderMV, 0));
...
float PatchError(float2 uv_cur, float2 uv_prev, out float contrast)
{
    ...
    [unroll] for (int i = 0; i < 9; ++i)
    {
        ...
        p[i] = tex2Dlod(sDLSS5_PrevLuma, float4(SbsSampleUV(uv_prev + o, uv_cur, px), 0.0, 0.0)).x;
        mc += c[i]; mp += p[i];
    }
    mc /= 9.0; mp /= 9.0;
    float err = 0.0;
...
float4 ValidateTests(float2 uv, float2 mv)
{
    ...
    if (VALIDATE_LUMA)
    {
        ...
        {
            const float l = Luma(SbsSampleUV(uv + float2(x, y) * px, uv, px));
            lmin = min(lmin, l); lmax = max(lmax, l);
        }
        const float lp     = tex2Dlod(sDLSS5_PrevLuma, float4(SbsSampleUV(puv, uv, BUFFER_PIXEL_SIZE), 0.0, 0.0)).x;
    ...
    const float dc = LinearDepth(uv);
    if (VALIDATE_DEPTH && dc < 0.999)
    {
        const float dp  = tex2Dlod(sDLSS5_PrevDepth, float4(SbsSampleUV(puv, uv, BUFFER_PIXEL_SIZE), 0.0, 0.0)).x;
    ...
    {
        const float2 pmv   = tex2Dlod(sDLSS5_PrevMV, float4(SbsSampleUV(puv, uv, BUFFER_PIXEL_SIZE), 0.0, 0.0)).xy;
...
{
    if (!GeometryEnabled()) { A = 0.0; B = 0.0; return; }
    const float2 suv = (floor(vpos.xy) + 0.5) / float2(DLSS5_FIT_W, DLSS5_FIT_H);
    const float  d   = LinearDepth(suv);
                 ...
                 out float4 P3 : SV_Target3, out float4 P4 : SV_Target4, out float4 P5 : SV_Target5)
{
    ...
    if (!GeometryEnabled()) { P0 = 0.0; P1 = 0.0; P2 = 0.0; P3 = 0.0; P4 = 0.0; P5 = 0.0; return; }
    float p[18];
...
{
#if DLSS5_STEREO_DEPTH
    // Feeder must also skip evaluation and reset both eye histories when invalid.
    // These guards prevent accidentally supplying a stale or Generic Depth fallback.
    if (!ETS2_STEREO_DEPTH_VALID || !MotionHistoryReady()) {
        mv_out = 0.0; mask = 1.0; depth = 0.0; static_now = 0.0; return;
    }
#endif
    ...
    float2 mv = flow;
    float  distrust = 0.0;
    static_now = 0.0;   // invalid border history is not a static-hypothesis win
#if DLSS5_SBS
    if (SbsInvalidVector(uv, flow)) {
        mv_out = 0.0; mask = 1.0; depth = RawDepth(uv); return;
    }
#endif

    if (GeometryEnabled() && FitIsUsable())
    {
        const float  d = LinearDepth(uv);
    ...
    else if (MV_VALIDATE)
    {
        ...
        {
            const float won_before = tex2Dlod(sDLSS5_PrevStatic, float4(SbsSampleUV(uv, uv, BUFFER_PIXEL_SIZE), 0.0, 0.0)).x;
    ...
    depth  = RawDepth(uv);
#if DLSS5_SBS
    // Sign/scale are diagnostic output transforms, not eye-coordinate conversion.
    // Guard the actual vector delivered to NGX too. There is no extra factor of 2.
    if (SbsInvalidVector(uv, mv * MV_SIGN * MV_SCALE)) {
        mv_out = 0.0; mask = 1.0; static_now = 0.0;
    }
#endif
}
...
float3 PS_Debug(float4 vpos : SV_Position, float2 uv : TEXCOORD) : SV_Target
{
...
#if (DLSS5_STEREO_DEPTH && DLSS5_STEREO_DEPTH_REVERSED) || (!DLSS5_STEREO_DEPTH && RESHADE_DEPTH_INPUT_IS_REVERSED)
    ...
    {
#if DLSS5_MV_LOWRES
        return saturate(tex2Dlod(sDLSS5_ProviderConfidence, float4(ProviderSampleUV(uv), 0.0, 0.0)).x).xxx;
#else
    ...
    if (DEBUG_VIEW == 6)
    {
        const float2 pv = PredictMV(uv, LinearDepth(uv)) * BUFFER_SCREEN_SIZE;
    ...
    if (DEBUG_VIEW == 7)
    {
        ...
        const float4 g = GeometryDecide(uv, LinearDepth(uv), ProviderMV(uv));
